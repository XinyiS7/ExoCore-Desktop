...
export default function ProjectFilesDrawer({
 ...
 <>
  ...
  <div className="flex items-center justify-between px-4 py-3 border-b border-exo-mist-10 shrink-0">
   ...
   <button onClick={onClose} className="p-1 tx-body-mute hover:tx-body-normal transition-colors">
   <X size={14} />
   </button>
  </div>
  ...
  <div className="flex-1 overflow-y-auto p-4 space-y-6">
   {/* ── Work Directory ── */}
   <div>
   <h3 className="text-[0.625rem] font-mono tracking-[0.2em] tx-body-mute opacity-50 mb-2">
    Work Directory
   </h3>
   {hasWorkDir ? (
    <div>
    <p className="text-[0.625rem] font-mono tx-body-mute opacity-30 mb-2 truncate">{project.work_dir}</p>
    {treeLoading && !fileTree ? (
     <div className="flex items-center gap-2 text-xs tx-body-mute opacity-40 italic py-2">
     ...
     <div className="text-xs tx-body-mute opacity-40 italic">无法读取目录</div>
    )}
    </div>
   ) : (
    <p className="text-xs tx-body-mute opacity-30 italic">未绑定工作目录</p>
   )}
   </div>
   ...
   <h3 className="text-[0.625rem] font-mono tracking-[0.2em] tx-body-mute opacity-50 mb-2">
    ...
    <span className="ml-1 tx-body-mute opacity-30">· {projectFiles.length}</span>
    )}
   </h3>
   {projectFiles.length === 0 ? (
    <p className="text-xs tx-body-mute opacity-30 italic">无参考文件</p>
   ) : (
    <div className="space-y-1">
    {projectFiles.map(f => (
     <div
     key={f.id || f.name}
     className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-exo-accent/[0.04] transition-colors text-xs tx-body-mute opacity-70"
     >
     <FileText size={12} className="tx-body-mute opacity-40 shrink-0" />
