 ...
 * - Work Directory files (if project.work_dir is set)
 * - Project Files (uploaded / vectorized reference files)
 */
export default function ProjectFilesDrawer({ isOpen, onClose, project, projectFiles = [] }) {
    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-[350px] max-w-[90vw] bg-chat-panel border-l border-white/5 shadow-2xl z-50 flex flex-col animate-fade-in overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {/* ── Work Directory ── */}
          <div>
            <h3 className="text-[10px] font-mono uppercase tracking-[0.2em] text-chat-muted/50 mb-2">
              Work Directory
            </h3>
            {hasWorkDir ? (
              <div>
                <p className="text-[10px] font-mono text-chat-muted/30 mb-2 truncate">{project.work_dir}</p>
                <div className="text-xs text-chat-muted/40 italic">
                  Directory listing will be fetched in real-time.
                </div>
              </div>
            ) : (
