...
export default function KeyPoolSection({ platform, keys, loading, onKeysChanged }) {
 ...
 <div className="bg-chat-panel border border-white/5 rounded-lg p-3 space-y-3">
  ...
  <div className="flex items-center gap-2">
   ...
   <span className="text-[10px] font-mono tracking-[0.12em] text-chat-text/70">
   Key Pool
   </span>
  ...
  {!showAddForm && (
   ...
   disabled={saving}
   className="flex items-center gap-1 px-2.5 py-1 text-[9px] font-mono tracking-wider text-chat-accent/60 hover:text-chat-accent border border-chat-accent/15 hover:border-chat-accent/30 rounded transition-all disabled:opacity-30"
   >
   <Plus size={11} /> Add
   </button>
  )}
  ...
  <div className="flex items-center gap-2 p-2 bg-chat-bg border border-white/8 rounded">
   ...
   disabled={!canAdd}
   className="px-3 py-1.5 bg-chat-accent text-white text-[9px] font-bold tracking-[0.12em] rounded hover:brightness-110 disabled:opacity-20 disabled:grayscale transition-all flex items-center gap-1 flex-shrink-0"
   >
   {saving ? (
   ...
   >
    ...
    {editing === k.alias ? (
     ...
     {editMode === 'alias' && (
      ...
      disabled={saving}
      className="text-[8px] font-mono tracking-wider text-chat-accent/40 hover:text-chat-accent transition-colors px-1"
      >
      Key
      </button>
     )}
