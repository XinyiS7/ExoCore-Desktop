...
export default function ProjectDetail({ appState, setView, goBack, viewParams }) {
 ...
 <div className="flex-1 h-full flex flex-col bg-exo-bg overflow-hidden">
  ...
  <div className="flex items-center gap-2">
   ...
   <button
   onClick={() => goBack()}
   className="hidden md:flex items-center gap-1.5 px-3 py-2 text-exo-muted hover:text-white hover:bg-white/5 rounded-md transition-all text-xs font-mono tracking-wider"
   title="Fold"
   >
  ...
  <div className="flex items-center justify-between mb-1">
   <span className="text-[10px] font-mono tracking-[0.3em] text-exo-muted">Project Prompt</span>
   {!editingPrompt ? (
   <button
    onClick={() => { setPromptDraft(project?.prompt || ''); setEditingPrompt(true); }}
    className="text-[10px] text-exo-muted hover:text-exo-accent transition-colors font-mono tracking-wider"
   >
    Edit
   </button>
   ) : (
   <div className="flex items-center gap-2">
    ...
    className="text-[10px] text-exo-accent hover:text-white transition-colors font-mono tracking-wider"
    >
    Done
    </button>
    <button
    onClick={() => setEditingPrompt(false)}
    className="text-[10px] text-exo-muted hover:text-white transition-colors font-mono tracking-wider"
    >
    Cancel
    </button>
   </div>
  ...
  <div className="flex-1 md:w-1/2 md:flex-none overflow-y-auto scrollbar-hide border-b md:border-b-0 md:border-r border-exo-mist-8 p-4 md:p-6">
   ...
   <p className="text-[10px] font-mono tracking-[0.3em] text-exo-muted">Sessions</p>
   {sessions.length > 0 && (
  ...
  <div className="flex-1 md:w-1/2 md:flex-none overflow-y-auto scrollbar-hide p-4 md:p-6">
   <div className="flex items-center justify-between mb-3">
   <p className="text-[10px] font-mono tracking-[0.3em] text-exo-muted">Files</p>
   <button
    onClick={() => fileInputRef.current?.click()}
    disabled={isUploading}
    className="flex items-center gap-1.5 text-[10px] text-exo-muted hover:text-exo-accent transition-colors font-mono tracking-wider"
   >
    <Upload size={12} />
