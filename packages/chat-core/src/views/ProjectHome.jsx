...
export default function ProjectHome({ appState, setView }) {
 ...
 <div className="flex-1 h-full flex flex-col bg-exo-bg overflow-hidden">
  ...
  {unassignedSessions.length === 0 ? (
   <div className="flex items-center justify-center h-full tx-system-mute">
   <div className="text-center">
    <MessageSquare size={24} className="mx-auto mb-2 opacity-20" />
    <p className="text-xs font-mono">No unassigned sessions</p>
    <p className="text-[0.625rem] tx-system-mute opacity-50 mt-1">Create a new chat or select a project</p>
   </div>
   </div>
  ) : (
   ...
   {unassignedSessions.map(s => (
    ...
    >
    <div className="p-2 rounded-md bg-white/[0.03] border border-exo-mist-10 tx-system-mute group-hover:tx-system-accent group-hover:border-exo-accent/20 transition-all">
     <Hash size={14} strokeWidth={1.5} />
    </div>
    <div className="flex-1 min-w-0">
     <p className="text-xs font-medium tx-system-normal truncate">{s.name || `Session #${s.id}`}</p>
     <p className="text-[0.5625rem] tx-system-mute mt-0.5">
