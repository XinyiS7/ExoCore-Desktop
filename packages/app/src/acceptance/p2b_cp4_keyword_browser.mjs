...
const root=resolve('packages/app/dist'), out=resolve('Plan/.p2b-acceptance-shots/cp4-keyword-r2');
...
const server=createServer((req,res)=>{
  ...
  if(pathname.startsWith('/api/')) {
    ...
    let data;
    if(pathname==='/api/core/projects/41/files/') data=[]; else if(pathname==='/api/memory/knowledge/') data=[{id:201,title:'Focus fragment',source_type:'obsidian_md',tags:[],keywords:['longkeyword'.repeat(25)],abstract:'original',updated_at:'2026-01-01T00:00:00Z'}]; else if(pathname==='/api/core/projects/') data=[project];
...
try {
  ...
  for(const width of [320,1280]){
    ...
    await until("!!document.querySelector('.project-knowledge-row button')");await click('.project-knowledge-row button');await until("document.activeElement?.id==='knowledge-abstract'");
    const metrics=await evaluate("(()=>{const chip=document.querySelector('.project-keyword-chip'),body=document.querySelector('.app-dialog-body'),button=chip.querySelector('button');const cr=chip.getBoundingClientRect(),br=body.getBoundingClientRect(),r=button.getBoundingClientRect();const range=document.createRange();range.selectNodeContents(chip.firstChild);const text=range.getBoundingClientRect();const x=r.left+r.width/2,y=r.top+r.height/2,hit=document.elementFromPoint(x,y);return {chipWidth:cr.width,textWidth:text.width,textFits:text.right<=cr.right+1,removeRight:r.right,bodyRight:br.right,removeVisible:r.width>0&&r.right<=br.right+1&&(hit===button||button.contains(hit)),whiteSpace:getComputedStyle(chip).whiteSpace,bodyScrollWidth:body.scrollWidth,bodyClientWidth:body.clientWidth};})()");
    observations.push({width,...metrics});console.log('KEYWORD_LAYOUT',width,metrics);
  }
  const result=observations.every(o=>o.textFits&&o.removeVisible)?'PASS':'FAIL';writeFileSync(join(out,'observations.json'),JSON.stringify({result,observations,requests},null,2));assert.equal(result,'PASS','keyword wraps and removal action remains visible');
