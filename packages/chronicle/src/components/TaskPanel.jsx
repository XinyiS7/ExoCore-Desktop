...
function SectionHeader({ label }) {
  return (
    <div className="px-6 pt-6 pb-2 text-[0.725rem] tracking-[0.2em] font-bold text-exo-muted/40 font-mono border-b border-exo-mist-6 mb-2">
      {label}
    </div>
  );
}
    ...
    <div className="flex h-full overflow-hidden relative bg-exo-bg">
      ...
      `}>
        <div className="flex items-center justify-between px-4 h-14 border-b border-exo-mist-10 lg:hidden">
          <span className="text-[0.725rem] font-bold text-exo-accent tracking-[0.2em] font-mono">CHRONOS_FLTR</span>
            ...
            <div className="space-y-1">
              {TYPE_TABS.map(t => (
                <button
                  ...
                  className={`
                    group flex items-center gap-3 w-full text-[0.7875rem] py-2 px-3 rounded-[2px] transition-all text-left font-mono tracking-tight
                    ${typeFilter === t.value
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-noise">
        ...
        <div className="flex items-center justify-between px-4 lg:px-8 h-14 border-b border-exo-mist-10 flex-shrink-0 bg-exo-pure/40 backdrop-blur-md">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-[0.725rem] tracking-[0.3em] text-exo-accent font-bold font-mono">Chronos / {selectedDate}</span>
              <span className="text-[0.6rem] tracking-[0.1em] text-exo-muted font-mono opacity-40">Operational Pipeline Monitor</span>
            </div>
          </div>
          
          <button
            onClick={() => setModalEntry({})}
            className="flex items-center gap-2 px-4 py-1.5 bg-chron-accent text-white rounded-[2px] text-[0.7875rem] font-bold tracking-widest hover:bg-chron-accent/80 transition-all shadow-brutalist active:scale-95"
          >
            <Plus size={14} />
        ...
        <div className="flex-1 overflow-y-auto px-4 lg:px-8">
          {loading && (
            <div className="flex items-center justify-center h-48 text-[0.7875rem] text-exo-muted font-mono tracking-[0.3em] animate-pulse">
          ...
          {!loading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center h-64 opacity-20">
              <span className="text-[0.7875rem] text-chron-text/40 font-mono tracking-[0.3em]">No Active Tasks</span>
              <span className="text-[0.6625rem] text-chron-text/30 font-mono tracking-[0.1em] mt-2">Cycle: {selectedDate}</span>
            </div>
          )}
            ...
            <div className="pb-20 lg:pb-12 pt-4">
              ...
              {pinned.length > 0 && (
                <div className="mb-8 border border-exo-accent/30 bg-exo-accent/[0.03] rounded-[4px] overflow-hidden shadow-[0_0_30px_rgba(212,175,55,0.05)]">
                  <div className="px-4 py-2 bg-exo-accent/10 border-b border-exo-accent/20 text-[0.725rem] tracking-[0.2em] font-bold text-exo-accent font-mono flex items-center gap-2">
